...
//AUTHOR:		TKBS
//DATE:			04 04 2025
//INFO:			Provides More than just cheat codes
...
// -----------------------------------------------------------------------------------------
// ---- GAME Version: [PS2] 2002 marketting build & HYBRID  || CRC: 09AAE688 & 31A67C86 ----
// -----------------------------------------------------------------------------------------


//----------------------------------------------------------------------------------------------------------
//-------------------------------- Work-around Issues: prevent start-up crash & hangs ----------------------
//----------------------------------------------------------------------------------------------------------

// This stuff was related to using gta vice city. Not sure if it is needed anymroe.

// -- (prevent start-up crash & hangs) -- 		
// * Attempt to fix Skip Intro (crash & hangs) mpeg movies - beta does not have l1,l2,r1,r2 skip function.
// * || can be ignored. Attempt to fix crash & hangs when trying to Skip Intro mpeg movies - beta does not have l1,l2,r1,r2 skip function.
//patch=1,EE,80001040,extended,0C0013B0
//patch=1,EE,8000EBE0,extended,0C0013B0

//todo: (workaround) issue: 0x00000000 || pcsx2 Invalid macroblock_address_increment code(0x00000000)
//patch=1,EE,00000000,extended,00000000

//todo: 002DBA90 - some issue - hit store breakpoint here to check it- unsolved.
...
//Infinite Ammo	[Game Cheat version]			|| Ported from my PAL 2003 version. 	|| SAME ISSUES AS 100% COMPLETE - the infinite ammo does not actually work ????
//Lines 1-4 blank mem space ?
//Line 5 - Default value 0C0C28D0 --> 0C03FFC0	|| Instruction: "sar bh,03"
//error in the log for this: //ERROR: ==  (EE pc:000FFF00) TLB Miss, addr=0x2a77 [store]
...
//patch=1,EE,202D6DF4,extended,0C03FFC0

//* joker commands unknown, * SEE IMAGE For JOKER-COMMAND-BASE-ADDRESS maybe?
//joker maybe DFB00000 ?

//* Ignore ---- Unless you can solve it - "The .elf file cheats"

//* From .elf File: UDUDUD..T3T3T3..LRLRLR..ULULUL..242424..
//004b5d18 69646769
//004b53c0 4c554c55
//patch=1,EE,004b5d18,extended,69646769
//patch=1,EE,004b53c0,extended,4c554c55

//T3T3T3 - unknown
//242424 - unknown

//URDLURDL........DLDLURDL........DDDLURDL........ULDRULDR........XDXUXRXL........22442244........33113311........11331133
//----------------------------------------------------------------------------------------------------------

//--------------------------------------------------------------------------------------------------------------------
// ---- Blood-,Dirt-,Hot-Ring Arena Races ---- THis worked. then as usual i tried to record an example of it and it failed.
//--------------------------------------------------------------------------------------------------------------------
// * Remove other vehicles to race around easy in 1st postiion.

// * "NOP" These to remove other vehicles from the hotring, bloodring arena races.
// [address] - [bytes] 				- OPCODE
// 300F78CA - 89 11                 - mov [ecx],edx		|| 300F78CA - 90                    - nop
// 300F78CC - 31 C0                 - xor eax,eax		|| 300F78CC - 90                    - nop
// * then set position number with these
//patch=1,EE,2055BA80,extended,00000001
//patch=1,EE,20B9C308,extended,00000001

// * Result: BloodRing Always 1st Position  ||Default Values == C0311189 || A3C03111
//patch=1,EE,300F78CA,extended,90909090
//patch=1,EE,300F78CB,extended,A3909090

//* Then reset back to default and continue racing with no other other vehicles.
//patch=1,EE,300F78CA,extended,C0311189
//patch=1,EE,300F78CB,extended,A3C03111

//HOTRING - test 2 vehicles are slow, always 1st - but laps stuck on 1
//300F3530 - 90                    - nop  			|| C0319090
//300F14A2 - 90                    - nop  			|| C0319090
//300F3530 - 90                    - nop  			|| C0311189
//300EDD68 - 89 11                 - mov [ecx],edx 	|| C0311189

//HOTRING
//patch=1,EE,300F3530,extended,C0319090
//patch=1,EE,300F14A2,extended,C0319090
//patch=1,EE,300EDD68,extended,C0319090




// -----------------------------------------------------------------------------------------
// ---- GAME Version:	[PS2] 2003 (PAL) (2.03) (SLES-51061) || CRC: CFCB0D20 ----
// -----------------------------------------------------------------------------------------

// ---- Notes: ---- "Nibbles & bits, bytes, hex decimel":
// 2 nibbles = (4 bits) instead of 1 byte. || 4 bits per digit || 9 dec  = 1001 binary ||
// Example: to make both digits 9:  value = 10011001 (0x99 hex / 153 decimal).
// Example: 72 decimel = 01110010 binary (or 0x72 hex / 114 decimal).

// ---- Input Mappings:  ---- Input Codes (Button): ----
0000FFEE & 	0000FFFF 	== Select + UP
0000FF7E	0000FFFF	== Select + LEFT
0000FABF	0000FFFF	== L1+L2+DOWN
0000FEFD	0000FDFD	== L3+L2 | L3+R2 To Stop
0000FFED	0000FFFF	== L3+UP
...
//Press select and up ?			= //patch=1,EE,D05C1702,extended,0000FFFF
//Press (L3+RIGHT+RIGHT)		= //patch=1,EE,D05C1702,extended,0000FFEE
//0000FABF   					= DOWN
//0000FAEF  					= UP
//patch=1,EE,D05C1702,extended,0000FCEF - L2+R2+UP
//patch=1,EE,D05C1702,extended,0000FCFE - L2+R2+Select
//patch=1,EE,D05C1702,extended,0000FCBF - L2+R2+DOWN

Examples:
//Decrease Speed 		- (Select + LEFT)
patch=1,EE,D05C1702,extended,0000FF7E
patch=1,EE,202789B8,extended,00000000
patch=1,EE,D05C1702,extended,0000FFFF
patch=1,EE,202789B8,extended,14400018

//Increase Speed 		- (L1+L2+DOWN)
patch=1,EE,D05C1702,extended,0000FABF
patch=1,EE,20278948,extended,00000000
patch=1,EE,D05C1702,extended,0000FFFF
patch=1,EE,20278948,extended,14400018

// ---- Master Codes ---- // For PS2 Cheat Devices.
//MASTER Enable Code: patch=1,EE,901220EC,extended,0C041C64

// ---- Quick Access: Notes: ----
//Ranges
//	- Most codes: 			20000000-21FFFFFF
//	- Controls, functions: 	00000000-20000000
//	- Rarely used : 		30000000-30000000+
//	- emulator "20######" can sometimes be replaces with "00######" or "10######"
	
// Free Memory:	
//* Some notes included on free memory spaces.
...
//- Converting Codes: -
//					a list of offsets and simple equations that - although trial & error, guess work, - have led to discovering stuff.
//					when an offset works for one convert but not another, the likelihood of the correct address being within a small range of that offset is high.
...
//Had i known about the beta years ago, i would have realised then that was already at the memory region for the debug menu.
...
** Garages do not work the same as heli-pads!

New Notes:
VH ID ---- X ---- Y ---- Z ---- H ---- ForceSpawn? ---- Immediate Spawn ---- DMG Proof ---- Colours ---- FF ----
...
//**** Force DTS Audio "FEA_DTS" Enabled ****				- GTA Vice City PS2 using DTS (for Starter Save PS2 & PS3 Users)
//Notes: DTS Is not blocked in the Beta.
//Notes: PCSX2 Devs neever released their private DTS build, which would be great for VC.
...
// ---- Cheat Addresses: ----
//* Cheat Addresses: When you activate a cheat This values Count 0,1,2,3 etc
//* When you die, they reset to 0... Maybe "Number of active cheats?"
//* No idea what it is. But it cannot be Number of cheats used, so maybe these write to that address wherever it is ???
//20B510B4
//20B510B8
//20B510CC
//20B510D0
...
// ---- Weapons Addresses: ----
//Weapons: Curently Equipped:
//** MEMORY REGION FOR WEAPONS, display image, ammo, firing rate etc.
//Line 1 - Last Byte = "0-8" 	|| Default = 80640000
//patch=1,EE,20B511A8,extended,80640000
//Line 2 - Last 3 Bytes =  		|| Default = FFFFFFFF
//patch=1,EE,20B511D4,extended,FFFFFFFF
//Line 2 Values
//0000010C
//0000010E
//00000113
...
** These are dynamic addresses that change, -- require pointer scans.
** a save state can be used to keep the same addresses, but that requires the player to start their game using the starter save state and to always save at the end with a save state and reload that.

//Weapons: Ammo - Molotov Ammo
//patch=1,EE,20B510E8,extended,0000029A

//Weapons: Rocket Launcher Ammo 		- Addresses Change - use save game.
//patch=1,EE,20B51160,extended,0000029A
//Super Fast (Firing Rockets): ALWAYS 4 bytes before.
//patch=1,EE,20B5115C,extended,0E300008

//Weapons: Rocket Launcher
//patch=1,EE,20B50A60,extended,0000029A
//Weapons: Rocket Launcher Super Fast Firing Rockets:
//patch=1,EE,20B50A60,extended,0E300008

//Equip Rocket Launcher With 666 Ammo (0000029A):
// [20B51150] +4 Bytes || --> 10101000 0000001E 00000000 00000001 0000029A 00000000 10101000 0000001D

//Super Rocket Launcher:
//patch=1,EE,20B51150,extended,10101000
//patch=1,EE,20B51154,extended,0000001E
//patch=1,EE,20B51158,extended,00000000
//patch=1,EE,20B5115C,extended,0E300008
//patch=1,EE,20B51160,extended,0000029A

// ---- TESTS ---- (TEST Unknown effect codes). ---- UNKNOWN & List of possibles: ----
//* Peds in thriller outfits | zombie police | return game speed to normal | ER never arrive
//* idiot AI | Lights Always Green, red, amber | Wheel size? | Girls & peds get in your car | Smoke a cigarette
...
//UNKNOWN: Vehicles: Traffic Lights Always Green?
//patch=1,EE,20278CB4,extended,1440000E

//UNKNOWN: Vehicles: Traffic Lights Always amber?
...
//* from searcing up memory region where armour and others cheats are.
//* this switched dualshock on or off or soemthign, kicks user to controller options.
//patch=1,EE,20277E38,extended,1460000A
...
//UNKNOWN - probably failed vehicle spawn testing
...
//unknown version:  Never Lose Weapons
//patch=1,EE,201B4480,extended,080B4DF9

// Not Tested: Unknown: Does something - screen flash in emu indicates may do summin. - 20278818 1440000F
//patch=1,EE,20278818,extended,00000000

//Water Wheels - probably partice effects From that image in my folder.
...
//--------------------------------------------------------------------------------------------------------------------
//---- Some Information about values and stuff ----
//--------------------------------------------------------------------------------------------------------------------
...
//Heli.FireHunterGun() || hex: 02 30 09 06 33 B3 50 C4		- Just a reference guess.
//Fire Helicopter gun
patch=1,EE,20556784,extended,06093002
patch=1,EE,20556784,extended,C450B333

Chaning the value at the end of this address causes the game to set tommy health to 0 and show links view
204B1B78 00000000
...
// Some stuff from debug dissamebler thing in PCSX2
// 002749E0 1083FFF9

// this probably somthing to do with loading - the region ? 244200||01 ||02|| 03 etc
patch=1,EE,002749F4,extended,24420001
...
//wtf: also wtf: there is a limit to number of lines? 1024 old verisons, 2048 newer?
