...
// Free Memory:	* Some notes included on free memory spaces.
...
---- TODO: CODES NEEDED! ----
//TODO: Glass Break (toggle) | Tyre Break Toggle - Your Vehicle, All Vehicles.  - See "Bullet Proof Tyres" & vehicle notes.
//TODO: Change FBI texture to xfiles mulder PS2 texture dump. || TODO: re-name char model cheats to correct names.

---- LIST OF CODES NEEDED! ----

** Main Concerns is not Having the following;- **

1. Joker Commands for Pal Beta 2002. 
2. Vehicle LOD switch removal : - 	broken or not working correctly, and this is an important one, low LOD's need to be stopped.
3. garage editor - all garages, vehicles, helipads road parking, carparks, CAR_GENERATORS etc etc.
	* boats are cars
	* Boats & helis with wheels etc	? See "SunShine Auto's Spray N Go" glitch - how to permanent feature it?
	- sirens (toggle),
	- full damage proof (i.e. No Tank explode etc.)	
4. set all (colour) cars e.g. white, not just black, purple			- ???
...
//		- UI stuff, like changing property cost shown on purchase to that of the new values. (1 instead of 7000) some done but not all
		...
		//* Add new Garages on Beach -or replace the beach huts witha garage?
			visuals, fov, camera control, etc.				-	partial
			sky toggle, models toggle,						- 	partial broken or not working or not done.

9. Particle Effects swap: tested and worked but not saved as i had to many random changes on the go.
//...^^  DIRT | WATER_WHEEL | BOOD_SPURT | DEBRIS ... etc etc. See "CHANGE a PARTICLE EFFECT" Blood for Water Swap example   OR...CT file.


10. * UNKNOWN CHEATS ACTIVATED ?
11. pedestrian modifiers
12. cop stuff
	//* Set #of helicopters chasing you
	//* Set # wanted stars //Set WANTED stars 1-6.
	//* idiot AI

13. pop up notifications
...
** These are addresses that change, and require pointer scans, which maskes no sense to me.
...
//* Peds in thriller outfits | zombie police | return game speed to normal | ER never arrive | idiot AI | Lights Always Green, red, amber | Wheel size? | Girls & peds get in your car | Smoke a cigarette
//* These are 'Anything' not already listed (Missing Codes List)
...
// Unknown: Does something - screen flash in emu indicates may do summin. - 20278818 1440000F
...
//Damage Proof ->		//patch=1,EE,206B608C,extended,0000001F
