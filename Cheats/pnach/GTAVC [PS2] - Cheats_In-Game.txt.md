TITLE:		GTA Vice City PS2 In-Game Cheats
** ALSO some work for San Andreas **
TITLE:		GTA San Andreas PS2 In-Game Cheats

---- GTA Vice City Glitches ----
[Bullet proof tyres (ICF) Glitch]
** Bullet proof tyres - Ice cream factory (ICF)
* Performing the Vigilante + Save icon Glitch at the Ice cream factory will give your vehicle Bullet Proof tyres.
* Bullet-Proof Tyres are lost when put in a garage.

[Tyres for Helicopters Glitch]
* Using the Auto Shine Pay'n'Spray (spray-n-go) will give your heli Wheels.
* Tyres are lost when put in a garage.

[Vigilante + Save icon Glitch]
* a method of duplciating mission events e.g: vehicle spawns, to obtain those vehicles.
* this process is entirely pointeless except for the bullet proof tyres glitch mentioned above.
* use the Hybrid Cheats & Debug Version, .pnach and .ct files to use garage or heli-pad vehicle editors instead of wasting time glitching missions.

----------------------------------------------------------------------------------
				---- VICE_CITY [PS2] Hybrid/ Beta Only ----
----------------------------------------------------------------------------------
In-Game: 
	Debug Menu: 						L1+ R1 on the menu screen.
	Wanted Level + Up					Left, Right, (repeated)
	Wanted Level - Down					Up, Down,  (repeated)	

---- Cheats & Debug ----
[In-Game] [HARDCODED]
PAD 1 - Debug Menu: 		== L1+ R1 on the menu screen.
PAD 1 - Wanted Level + Up	== Left, Right, (repeated)		|| LRLRLR
PAD 1 - Wanted Level - Down	== Up, Down,  (repeated)		|| UDUDUD

PAD 2 - HUD TOGGLE 			== START
PAD 2 - CIRCLE  / R2 		== (CAMERA POSITIION = CIRCLE) && (TELEPORT = R2) (to spawn at cam position)

PAD 2 - stats pages			== L1 + L2 + R1 + R2
PAD 2 - show coords/ player position && mem 	== 	X, Triangle, Square, Cirlce, & R2

---- Debug Mode (ON OR OFF) ----
PAD 2 - Triangle 			== Toggle: debug
PAD 2 - DOWN 				== weapons
PAD 2 - LEFT/RIGHT			== MISSION NUMS UP/DOWN  ...then press R3
PAD 2 - R3 					== SPAWN at mission location etc.

---- Debug Mode OFF ----
PAD 2 - L1 & L2	In Vehicle  == Vehicle colours 1 & 2:- 0-94 (CYCLE) & ** DMG PROOF, WaterTight, No Upside-Down Damage, Invulnerable Tyres,
PAD 2 - DOWN 				== SPAWN Weapons
PAD 1 - SELECT + L1 		== Weather: 1-7 (CYCLE) (6 is nomral + hurricanes || 7 is normal - No hurricanes)

---- Debug Mode ON ----
PAD 2 - R1  				== SPAWN VEHICLE ###	|| Hold x decrement || ...- no need for this but -- hold square increment in jumps of 11?
PAD 2 - L1 || L2 			== Toggle: set_everyone_ignore_player | set_police_ignore_player etc
PAD 2 - SELECT + X 	 		== Clothes 1-12 (CYCLE) + bonus PLAY13 custom.
PAD 2 - SELECT 				== CleanStreets ON|| OFF - Toggle cars and peds

[Freeze (Toggle)]				== Freeze Player...
// PAD 1 - L1
// PAD 1 - L2
// PAD 2 - X				== ... then + R3 for extra cheats.

[extra cheats]
// When Frozen				== // Writing coordinates to file...
// PAD 1 - L1				** && will give you some £££ $$$ cash, money, bucks!**
// PAD 1 - L2				** && unlock secret garage **
// PAD 2 - X				* && other bonus stuff.
// PAD 1 - R3 				...

----------------------------------------------------------------------------------
				---- VICE_CITY [PS2] ----		---- SAN_ANDREAS [PS2] ----
----------------------------------------------------------------------------------
--------------- ** VICE_CITY ** ----------------------------------------------- || ------------------------- ** SAN_ANDREAS ** ----------------------------

---- Player Cheats ----
Health:[Heal] & Heal Vehicle:			R1 R2 L1 O ← ↓ → ↑ ← ↓ → ↑
Armor:                        			R1 R2 L1 ✕ ← ↓ → ↑ ← ↓ → ↑
Suicide:                              	→ L2 ↓ R1 ← ← R1 L1 L2 L1

----  Wanted-Level ----
Reset Wanted Level:                   	R1 R1 O R2 ↑ ↓ ↑ ↓ ↑ ↓
Increase Wanted Level By 2 Stars:     	R1 R1 O R2 ← → ← → ← →
Decrease Wanted Level By # Stars:     	***UNKNOWN***

---- Clothes ----
Random (also Removes Stars):			→ → ← ↑ L1 L2 ← ↑ ↓ →
Candy Suxxx:                          	O R2 ↓ R1 ← → R1 L1 ✕ L2
Hilary King:                          	R1 O R2 L1 → R1 L1 ✕ R2
Ken Rosenberg:                        	→ L1 ↑ L2 L1 → R1 L1 ✕ R1
Lance Vance:                          	O L2 ← ✕ R1 L1 ✕ L1
Love Fist  1:                     		↓ L1 ↓ L2 ← ✕ R1 L1 ✕ ✕
Love Fist  2:                     		R1 L2 R2 L1 → R2 ← ✕ ◻ L1
Mercedes:                             	R2 L1 ↑ L1 → R1 → ↑ O △
Phil Cassidy:                         	→ R1 ↑ R2 L1 → R1 L1 → O
Ricardo Diaz:                         	L1 L2 R1 R2 ↓ L1 R2 L2
Sonny Forelli:                        	O L1 O L2 ← ✕ R1 L1 ✕ ✕
Lance Vance  							O, L2, ←, X, R1, L1, X, L1

---- Weapons ----
Weapon Set 1:                         	R1 R2 L1 R2 ← ↓ → ↑ ← ↓ → ↑						||	SAME
Weapon Set 2:                         	R1 R2 L1 R2 ← ↓ → ↑ ← ↓ ↓ ↓						||	SAME
Weapon Set 3: Rocket Launcher          	R1 R2 L1 R2 ← ↓ → ↑ ← ↓ ↓ ←						||	SAME

---- Vehicle Spawns ----
Bloodring Banger 1:                   	↑ → → L1 → ↑ ◻ L2								||	Rancher
Bloodring Racer 2:           			↓ R1 O L2 L2 ✕ R1 L1 ← ←						||	SAME
Rhino Tank:                           	O O L1 O O O L1 L2 R1 △ O △						||	SAME
Love Fist Limousine:                  	R2 ↑ L2 ← ← R1 L1 O →							||	SAME
Hotring Racer 1:                      	R1 O R2 → L1 L2 ✕ ✕ ◻ R1						||	SAME
Hotring Racer 2:                      	R2 L1 O → L1 R1 → ↑ O R2						||	SAME
Romero's Hearse:                        ↓ R2 ↓ R1 L2 ← R1 L1 ← →						||	SAME
Trashmaster:                          	O R1 O R1 ← ← R1 L1 O →							||	SAME
Caddy:                                	O L1 ↑ R1 L2 ✕ R1 L1 O ✕						||	SAME
Sabre Turbo:                          	→ L2 ↓ L2 L2 ✕ R1 L1 O ←						|| 	NO EFFECT ?

---- Vehicle Modifier  ----
Cars Can Fly:                         	→ R2 O R1 L2 ↓ L1 R1							|| L2, R1, ◻, ↓, △, L1, L2, L1, △, X, X, L1
Cars Can Hop - Press L3 (#2):         	△ R1 R1 ← R1 L1 R2 L1							|| SAME
Cars Float On Water:              		→ R2 O R1 L2 ◻ R1 R2							|| SAME
Cars Invisible :                 		△ L1 △ R2 ◻ L1 L1								|| SAME
Change Tire Size (#3):                	R1 ✕ △ → R2 ◻ ↑ ↓ ◻
Explode Vehicles:                 		R2 L2 R1 L1 L2 R2 ◻ △ O △ L2 L1					|| SAME
boats Hover								R2, O, ↑, L1, →, R1, →, ↑, ◻, △					|| SAME ?
perfect handling						△, R1, R1, ←,  R1, L1, R2,  L1					|| SAME ?
More Speed								→, R1, ↑, L2, L2, ←, R1, L1, R1, R1				|| SAME ?

---- Traffic ----
Black Traffic:                        	O L2 ↑ R1 ← ✕ R1 L1 ← O							|| SAME
Pink Traffic:                         	O L1 ↓ L2 ← ✕ R1 L1 → O							|| SAME
Aggressive Traffic:                   	R2 O R1 L2 ← R1 L1 R2 L2						|| SAME
Green Traffic Lights And Faster Cars: 	→ R1 ↑ L2 L2 ← R1 L1 R1 R1

----  Gameplay ----
Increase Game Speed:                  	△ ↑ → ↓ L2 L1 ◻
Decrease Game Speed:                  	△ ↑ → ↓ ◻ R2 R1
Time Runs Faster:                     	O O L1 ◻ L1 ◻ ◻ ◻ L1 △ O △

----  Weather ----
Default Weather:                      	R2 ✕ L1 L1 L2 L2 L2 ↓
Foggy Weather:                        	R2 ✕ L1 L1 L2 L2 L2 ✕
Sunny Weather:                        	R2 ✕ L1 L1 L2 L2 L2 △
Cloudy Weather:                       	R2 ✕ L1 L1 L2 L2 L2 ◻
Rainy And Stormy Weather:             	R2 ✕ L1 L1 L2 L2 L2 O

----  NPC & Pedestrians ----
Not working ???							Square, L1, Triangle, R2 Square, L1, L1.
Pedestrians in  MJ Thriller outfits		◻ L1 △ R2 ◻ L1 L1
Pedestrians Attack Themselves:   		↓ ← ↑ ← ✕ R2 R1 L2 L1
Pedestrians Attack Tommy :        		↓ ↑ ↑ ↑ ✕ R2 R1 L2 L2
Pedestrians Have Weapons:             	R2 R1 ✕ △ ✕ △ ↑ ↓
Police Zombies:                    		O L1 ↓ L2 ← ✕ R1 L1 → ✕
Your Crew are Chicks With Weapons:     	→ L1 O L2 ← ✕ R1 L1 L1 ✕
Chick magnet:                       	O ✕ L1 L1 R2 ✕ ✕ O △
Shows Media Level: 						O, L1, ↓, L2, ←, X, R1, L1, →, X

----  Unknown, Broken Not Working ----
UNKNOWN: 								R2 O ↑ L1 → R1 → ↑ ◻ △
NO EFFECT ?		      					△, O, △, ↑, ←, ↓, ↓, ↓, ↓
Police Return From Dead:				***WRONG CODE WAS HERE***


[Super Stoppie Trick (Faggio)]

* gain a good amount of speed.
* perform stoppie:
 - ← analog stick ↑ & hold ◻ & R1)
 - release ◻ & R1 then hold ↓ X.
 // this was online uncredited, but i have 2 better methods.
 // But apparently there is a rumoured bug that wheelies are easier travelling south, not north.
 // the handling.cfg can be modified - not just custom but the bfinject is fdifferent depending on release version, 
 // also better handling can be unlocked in-game. 
 
 Hint:
 1. GTA PS2 Supports Pressure sensitivity on button presses PS2 hardware - allows half button presses
 2. The faggio is set to always ride at an angle of -35degrees or +30, this can be set to 0 (straight) by modifying the handling.cfg

 3. wilson's wheelie's:
 i had a friend in 90s who could infini wheelie and stoppie, his method was a flick of stick with a half button press held ↓ at a slight rotation to compensate, dude could do a lap of the city without exaggeration. (next to impossible without a perfect PS2 controller, perfect doesn't mean brand new, likely that the pad stuck or had friction)
